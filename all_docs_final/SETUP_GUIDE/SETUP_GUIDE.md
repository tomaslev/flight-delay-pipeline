...
```bash
# Linux / Mac
export DATABASE_URL="postgresql+psycopg2://airlines:liora@localhost:5432/airlines_db"

# Windows (PowerShell)
$env:DATABASE_URL="postgresql+psycopg2://airlines:liora@localhost:5432/airlines_db"
```
...
> **Note:** Without a database, the dashboard uses 2,000 mock flights automatically.
...
app.py
  └── data.py → get_flights_df()
        ├── Tries PostgreSQL via SQLAlchemy (DATABASE_URL)
        │     └── Returns 100,000 random real flights
        └── Falls back → 2,000 mock flights (always works)
...
| Problem | Solution |
|---------|----------|
| Dashboard shows 2,000 flights | PostgreSQL not connected — check DATABASE_URL |
